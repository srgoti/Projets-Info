...
if __name__ == '__main__':
    ...
    if not osp.exists("dm16.py"):
        raise FileNotFoundError(
            "Le code source dm16.py n'a pas été trouvé. Vérifier que votre "
            "programme est bien dans le même répertoire que ce script, qu'il "
            "s'appelle bien dm16.py et que vous avez utilisé l'exécution en "
    ...
    try:
        if "dm16" not in sys.modules:
            _P = importlib.import_module("dm16")
        else:
    ...
    # Importer les tests
    if not osp.exists("tests_dm16.py"):
        raise FileNotFoundError(
            "Le fichier tests_dm16.py n'a pas été trouvé. Vérifier qu'il se "
            "trouve bien dans le même répertoire que ce script.")

    with open("tests_dm16.py", "rt", encoding="utf8") as tests_file:
