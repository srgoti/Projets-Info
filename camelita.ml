...
(* Merci à Pierre Weis, Xavier Leroy et Ruchira Datta pour la *) 
(* base de données.                                           *)
(*::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::*)
...
(* ToDo : Détermine si un élement appartient à une liste. Le
   type doit être 'a -> 'a list -> bool *)
let rec membre element liste =
	match liste with
	|[] -> false
	|tete :: _ when tete = element -> true
	|_ :: queue -> xmembre element queue
;;
...
(* ToDo : Renvoie la valeur associée à la clé dans la liste de
   couples (clé, valeur). Le type doit être :
  'a -> ('a * 'b) list -> 'b *)

let rec associe_de cle liste =
  match liste with
  | [] -> raise Pas_trouve
  | (key, valeur) :: queue when key = cle -> valeur
  | _ :: queue -> associe_de cle queue
;;

(* ToDo : Renvoie la valeur associée à la clé dans la liste de
   couples (clés, valeur) où clés est ici une liste de clés
   associées à une même valeur. On lève l'exeption `Pas_trouve`
   si la clé n'est jamais associée à aucune valeur. Le type de
   cette fonction doit alors être :
   'a -> ('a list * 'b) list -> 'b *)

let rec associe_dans_liste cle liste =
    match liste with
    | [] -> raise Pas_trouve
    | tete :: queue -> associe_aux cle tete queue
and
associe_aux cle (cles, valeur) queue =
	match cles with
	|[] -> associe_dans_liste cle queue
	|head :: _ when head = cle -> valeur
	|_ :: tail -> associe_aux cle (tail, valeur) queue
;;

(* ToDo : Renvoie première valeur associée à une clé d'une liste
   de clés dans la liste d'association de couples (clés, valeur)
   où clés est encore une liste de clés associées à une même
   valeur. On lève l'exeption `Pas_trouve` si aucune clé de la
   liste de clé n'est associée à aucune valeur dans la liste
   d'association. Le type de cette fonction doit être :
   'a list -> ('a list * 'b) list -> 'b *)

let rec associe_d_un_element key_list liste =
	match key_list with
	|[] -> raise Pas_trouve
	|cle :: autre_cles -> cle_donne cle liste autre_cles
and
cle_donne cle liste autres_cles =
	match liste with
	|[] -> associe_d_un_element autres_cles liste
	|t :: q -> correspondance cle t autres_cles q
and
correspondance cle (cles, valeur) autres_cles liste =
	match cles with
	|[] -> associe_d_un_element autres_cles liste
	|t :: _ when t = cle -> valeur
	|_ :: q -> correspondance cle (q, valeur) autres_cles liste
;;
...
(* ToDo : renvoie la lettre en minuscule si c'était une
   majuscule et la même lettre sinon, de type char -> char *)

let minuscule car =
	Char.lowercase_ascii car
;;

(* ToDo : applique la fonction précédente à toutes les lettres
   d'une chaîne de caractères, de type string -> string *)
let en_minuscules chaine =
	String.map minuscule chaine
	(* String.lowercase_ascii chaine *)
;;
...
(* ToDo : il reste de nombreuses chaînes de caractères réduites
   à une seule espace dans le résultat de la fonction
   précédente. La modifier pour supprimer ces chaînes réduites à
   une espace qui ne nous seront pas utiles. *)
...
(* ToDo : écrire une fonction length : 'a list -> int qui
...
(* ToDo : écrire une fonction nth : 'a list -> int -> 'a renvoie
   l'élément à un certain indice dans une liste, on pourra
   supposer que l'indice est valable *)

let rec nth liste n =
    match liste, n with
    | [], _ -> failwith "il n'y a pas d'élément car la liste est vide"
    | tete :: _, 0 -> tete
    |  _ :: queue, _ -> nth queue (n - 1) 
;;

(* ToDo : Tire un élément au hasard dans une liste, de type 'a list -> 'a. On pourra utiliser les deux fonctions précédentes
   ainsi que la fonction `Random.int` après avoir inféré son
   fonctionnement. *)
let au_choix_dans liste =
  (* En attendant, toujours prendre le premier élément *)
  nth liste Random.int(length liste);;
(*List.hd liste*)
;;
...
let repond_au_patient reponse =
  (* ToDo : supprimer la ligne suivante et décommenter celle qui 
     suit *)
  (*let r = reponse in*)
  ...
  if fin phrase then
    ...
    (* ToDo : supprimer la ligne suivante et décommenter ce qui
       suit *)
